...
pbxProject.prototype.checkScriptDuplicate = function () {
    const existingScriptNames = ['"initCrashlytics"', '"uploadDsymFiles"'];
    let duplicateFound = false;
    Object.values(
        this.hash.project.objects['PBXShellScriptBuildPhase']
    ).forEach((value) => {
        if (
            typeof value === 'object' &&
            existingScriptNames.includes(value.name)
        )
            duplicateFound = true;
    });
